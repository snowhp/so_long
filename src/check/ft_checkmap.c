...
/*   Updated: 2023/04/19 15:29:02 by tde-sous         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_checkmap(t_data *data)
{
	...
	while (data->map[y] && y < data->max_y)
	{
		if ((x == 0 || x == data->max_x - 1) && data->map[y][x] != '1')
			ft_exit("Missing Vertical Walls", EXIT_FAILURE, data);
		if (data->map[y][x] == 'P')
		{
			data->p_y = y;
			data->p_x = x;
		...
		else if (data->map[y][x] == '1' || data->map[y][x] == '0')
		{
			...
			ft_exit("Unrecognized characters", EXIT_FAILURE, data);
		if (charp > 1 || chare > 1)
	...
	if (data->nb_col == 0 || chare == 0 || charp == 0)
		ft_exit("Not enough characters on the map", EXIT_FAILURE, data);
	ft_mapflood(data);
}
